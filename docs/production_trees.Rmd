...
joined <- per %>%
  ...
  mutate(
    ...
    # Amazon = ifelse(Amazon_Walmart.x != 8, "Yes", "No"),
    Amazon = ifelse(grepl("1", Amazon_Walmart.x), "Yes", "No"),
    ...
    Inc_PerCapita = income_midpt.x / Number_Persons.x,
...
```{r}
est_tbl <- joined %>%
  mutate(Inc_PC_lt_80 = ifelse(Inc_PerCapita < 80000, "Yes", "No")) %>%
  select(
    N_HBSR, WeightCaliper,
    HHSize:VehSufficiency,
    -HHSize, -Age, -HH_IncCat, -Inc_PerCapita,
    Inc_PC_lt_80,
...
est_tbl <- joined %>%
  select(
    ...
    HHSize:VehSufficiency, -HHSize, -num_kids, -t_access
  )
...
est_tbl <- joined %>%
  select(
    ...
    HHSize:VehSufficiency, -Inc_PerCapita, -w_access, -t_access
  ) %>%
  filter(VehSufficiency == "v0")
...
est_tbl <- joined %>%
  select(
    N_HBSHP, WeightCaliper,
    HHSize:VehSufficiency, -Inc_PerCapita,
    -Amazon, -num_adults, -contains("access")
  ) %>%
  filter(VehSufficiency == "vi")
...
est_tbl <- joined %>%
  select(
    N_HBSHP, WeightCaliper,
    HHSize:VehSufficiency,
    -Vehicles, -w_access, -Inc_PerCapita, -t_access
  ) %>%
  filter(VehSufficiency == "vs")
