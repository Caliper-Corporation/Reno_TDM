...
```{r}
per <- read_csv(
  "data/_private/hh_survey_2023/input/persons.csv",
  col_types = cols(
    Other_Why_No_Travel = col_character()
  )
)
hh <- read_csv("data/_private/hh_survey_2023/input/households.csv") %>%
  rename(Sample_Number = HH_Sample_Number)
loc <- read_csv(
  "data/_private/hh_survey_2023/input/locations.csv",
  col_types = cols(
    Other_Type_of_Place = col_character(),
    Scooter_Cost = col_double()
  )
)
```
...
translate_table <- function(tbl){
  equiv_tbl <- read_csv("data/_private/hh_survey_2023/input/equiv_table.csv")
...
write_csv(to_geocode, "data/_private/hh_survey_2023/output/to_geocode.csv")
geocoded_pts <- read_csv("data/_private/hh_survey_2023/output/geocoded_points.csv") %>%
...
# Determine access mode for bus trips by looking for mode transfer ("X")
# trips before them. If none found, assume walk. Note: after review, no
# drive to transit trips were recorded in the survey.
access_mode <- convert_modes %>%
  arrange(Sample_Number, Person_Number, `Activity/Trip_Number`) %>%
  group_by(Sample_Number, Person_Number) %>%
  mutate(
    access_mode = sapply(row_number(), function(x) {
      mode_xfers_before <- mode_model[1:(x-1)][Activity_Type_Code[1:(x-1)] == 'X']
      if (length(mode_xfers_before) > 0) tail(mode_xfers_before, 1) else NA
    }),
    ...
    egress_mode = sapply(row_number(), function(x) {
      mode_xfers_after <- mode_model[(x+1):length(Activity_Type_Code)][Activity_Type_Code[(x+1):length(Activity_Type_Code)] == 'X']
      if (length(mode_xfers_after) > 0) head(mode_xfers_after, 1) else NA
    }),
    egress_mode = case_when(
      is.na(mode_model) ~ "",
      mode_model != "bus" ~ "",
      is.na(egress_mode) ~ "walk"
    ),
  ) %>%
...
#   group_by(Sample_Number, Person_Number) %>%
#   filter(Sample_Number == 5000000081) %>% # for testing single houses
#   mutate(
...
#   ) %>%
#   group_by(Sample_Number, Person_Number, chain_id) %>%
#   mutate(
#     num_xfers = n(),
...
trip_file <- mark_anchor %>%
  ...
  mutate(
    ...
    Persons_on_Trip = lead(Persons_on_Trip),
...
per_add_tour_counts <- per_calc_ptypes %>%
  left_join(
    hh_with_tazs %>% select(-WEIGHT_FCTR), by = "Sample_Number"
  ) %>%
...
trip_add_tod <- trip_file %>%
  mutate(
    trip_dep_mins = trip_dep_hr * 60 + trip_dep_min,
    tod = case_when(
      trip_dep_mins < 390 ~ "EA",
      trip_dep_mins < 510 ~ "AM",
      trip_dep_mins < 870 ~ "MD",
      trip_dep_mins < 1050 ~ "PM",
      TRUE ~ "NT",
    )) %>%
  select(-trip_dep_mins)
