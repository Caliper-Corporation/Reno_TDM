...
tripTbl <- read_csv("data/_private/hh_survey/output/output_trips.csv") %>%
  mutate(
    homebased = ifelse(orig_purp == "H" | dest_purp == "H", "HB", "NH"),
    tour_type = ifelse(tour_type == "work", "W", "N"),
  ) %>%
  filter(purpose != "N_HBH")
```
...
calc_time <- tripTbl %>%
  mutate(
    departure_time = dep_hr * 60 + dep_min,
    arrival_time = arr_hr * 60 + arr_min
    # Weight_Factor = 1
  )
v_all <- c()
for (i in 1:length(v_time)) {
  v_all[i] <- sum(
    calc_time$Weight_Factor[
      calc_time$departure_time < v_time[i + 1] & 
        calc_time$arrival_time >= v_time[i]
    ],
    na.rm = TRUE
  )
}
...
work_trips <- calc_time %>%
  mutate(work_trip = ifelse(tour_type == "W", 1, 0))

v_wrk <- c()
for (i in 1:length(v_time)) {
  v_wrk[i] <- sum(
    work_trips$Weight_Factor[
...
am_peak_from <- chron(times. = c("7:00:00"))
...
tod_factors <- tripTbl %>%
  filter(homebased == "HB") %>%
  group_by(tod, purpose) %>%
  summarise(Trips = sum(Weight_Factor)) %>%
  group_by(purpose) %>%
  mutate(
  ...
  select(trip_type = purpose, tod, factor) %>%
...
direction_tbl <- tripTbl %>%
  filter(homebased == "HB") %>%
  group_by(trip_type = purpose, tod, pa_format) %>%
  summarize(trips = sum(Weight_Factor)) %>%
  mutate(
    pct = round(trips / sum(trips), 4),
    tod = factor(tod, levels = c("AM", "MD", "PM", "NT"), ordered = TRUE)
  ) %>%
  filter(pa_format == 1) %>%
  select(-trips, -pa_format) %>%
  rename(pa_fac = pct) %>%
  arrange(trip_type, tod)
...
# For transit factors, just use all trips to ensure balance.
# Transit only trips don't balance exactly out-vs-in in the survey because people
# can take other modes to get home.
trns_dir_tbl <- tripTbl %>%
  filter(homebased == "HB") %>%
  group_by(tod, pa_format) %>%
  summarize(trips = sum(Weight_Factor)) %>%
  mutate(
    pct = round(trips / sum(trips), 4),
    tod = factor(tod, levels = c("AM", "MD", "PM", "NT"), ordered = TRUE)
...
In the model, the directionality factors are used to further improve the
period-specific skims. The easiest way to explain the improvement is using the PM
period as an example. 

In the PM period, 89% of home-based work trips in the survey are traveling back
home (AP format). Trip productions estimated by the model are in PA format and,
with a traditional approach, would see skim times from home to work, which are
not congested in the PM period. This unrealistic representation of travel time
would cause incorrect mode and destination choices to be predicted.
...
skim_facs <- tripTbl %>%
  mutate(
    tour_type = ifelse(tod %in% c("MD", "NT"), "All", tour_type),
    tour_type = ifelse(homebased == "NH", "All", tour_type),
    homebased = ifelse(homebased == "NH", "NHB", homebased),
    homebased = ifelse(tod %in% c("MD", "NT"), "All", homebased)
  ) %>%
  group_by(tod, homebased, tour_type, pa_format) %>%
  summarize(trips = sum(Weight_Factor)) %>%
  mutate(
    pct = round(trips / sum(trips, na.rm = TRUE), 3),
    pa_format = ifelse(pa_format == 1, "pa", "ap")
  ) %>%
  select(-trips) %>%
  pivot_wider(names_from = "pa_format", values_from = "pct") %>%
  mutate(
    ap = ifelse(tour_type == "All", .5, ap),
    pa = ifelse(tour_type == "All", .5, pa)
  ) %>%
  rename(period = tod) %>%
  relocate(ap, .after = pa)
...
hb_hov3_tbl <- tripTbl %>%
  filter(!is.na(mode), homebased == "HB") %>%
  # for this calculation, include large-party auto_pay and oth_auto
  # trips in hov3
  mutate(
    party_size = HH_Members + Non_HH_Members,
    mode = ifelse(
      mode %in% c("auto_pay", "oth_auto") & party_size >= 3,
      "hov3", mode
    )) %>%
  filter(mode %in% c("hov3")) %>%
  group_by(purpose, tod, mode) %>%
  mutate(
    Value = round(weighted.mean(party_size, w = Weight_Factor), 3),
    Samples = n(),
  ) %>%
  ...
  mutate(
    daily_value = round(weighted.mean(party_size, w = Weight_Factor), 3),
    Value = ifelse(Samples < 30, daily_value, Value)
  ) %>%
  group_by(purpose, tod, mode) %>%
  slice(1) %>%
  select(purpose, tod, mode, Value, Samples) %>%
  ungroup() %>%
  arrange(purpose, tod, mode) %>%
  select(purpose, mode, tod, Value) %>%
  filter(mode == "hov3") %>%
  select(-mode) %>%
  rename(hov3 = Value, trip_type = purpose)

nhb_hov3_tbl <- tripTbl %>%
  filter(!is.na(mode), homebased == "NH") %>%
  # for this calculation, include large-party auto_pay and oth_auto
  # trips in hov3
  mutate(
    party_size = HH_Members + Non_HH_Members,
...
display_tbl %>%
  select(`Trip Type` = trip_type, Period, HOV3) %>%
...
result, TNC must be converted into those classes. The table below shows the
homebased trip factors calculated from the survey used to do this.
...
hb_oth_shares <- tripTbl %>%
  ...
  select(
    purpose, tod, mode, mode2, party_size,
    weight = Weight_Factor
  ) %>%
  mutate(
    mode2 = factor(mode2, levels = c("sov", "hov2", "hov3"), ordered = TRUE)
  ) %>%
  group_by(purpose, mode2) %>%
  summarize(weight = sum(weight)) %>%
  mutate(pct = round(weight / sum(weight), 4)) %>%
  select(trip_type = purpose, mode2, pct) %>%
...
nhb_oth_shares <- tripTbl %>%
  filter(tour_type != "H", !is.na(mode), homebased == "NH") %>%
  filter(mode %in% c("tnc")) %>%
  mutate(
    party_size = HH_Members + Non_HH_Members,
    mode2 = case_when(
  ...
  select(
    tour_type, tod, mode, mode2, party_size,
    weight = Weight_Factor
  ) %>%
  mutate(
    mode2 = factor(mode2, levels = c("sov", "hov2", "hov3"), ordered = TRUE)
...
write_csv(trns_dir_tbl , "data/output/tod/directionality_factors_transit.csv")
write_csv(skim_facs, "data/output/tod/directionality_skim_factors.csv")
