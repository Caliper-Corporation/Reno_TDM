...
The first step in synthesizing the population of residents in the model is to 
...
opted to use stratification curves in the model. These curves encode information
on the extent to which, for example, a zone with 3.5 persons-per-household on
...
In the model, the marginal distribution curves estimated are:
...
Community Survey (ACS) 5-year at the tract level. Vehicle and income information
is available from the ACS at the block group level.
...
se_tbl <- read_csv(
  "data/input/se_data/se_2022.csv",
) %>%
...
p <- ggplot(data = NULL, aes(x = avg_size, y = share, color = size)) +
  ...
  scale_x_continuous(lim = c(1, size_max)) +
  theme_bw()
...
workers <- acs_tract %>%
  mutate(
    avg_workers = (wrk0 * 0 + wrk1 * 1 + wrk2 * 2 + wrk3 * 3) / hh_tot
  ) %>%
  filter(hh_tot > 0) %>%
  select(GEOID, avg_workers, hh_tot, wrk0:wrk3) %>%
  group_by(GEOID) %>%
...
worker_min = 0
worker_max = 1.5
workers_functions <- fit_models(
  mean_workers, "avg_workers", "workers", 
  x = seq(worker_min, worker_max, 0.01),  degree = 3
) %>%
...
p <- ggplot(data = NULL, aes(x = avg_workers, y = share, color = workers)) +
  ...
  scale_x_continuous(lim = c(0, worker_max)) +
  theme_bw()
...
# source: https://data.census.gov/table?q=median+income&g=050XX00US32031
regional_median <- 84183
