...
estimate_nhb <- function(trips_df, trip_type, equiv = NULL, 
  ...
  logsum_type <- ifelse(
    v[length(v)] %in% c("lb", "bus"), "transit", logsum_type
  )
  logsum_tbl <- read_csv(
  ...
  temp1 <- collapse_types %>%
    mutate(personid = paste0(Sample_Number, "_", Person_Number)) %>%
    # only keeps trips of the same tour type (work/nonwork)
    filter(substr(trip_type, 1, 1) %in% c(tour_str, "y")) %>%
    select(personid, tour_num, trip_type, Weight_Factor, logsum) %>%
  ...
  combine_tbl <- bind_rows(temp1, temp2) %>%
    select(-keep) %>%
    filter(
      trip_type == "y" | !grepl("_NH", trip_type)
    )
  ...
  est_tbl <- combine_tbl %>%
    ...
    summarize(trips = sum(Weight_Factor)) %>%
...
apply_nhb_zonal <- function(trips_df, trip_type, coeffs) {
  ...
  nh_prods <- trips_df %>%
    filter(homebased == "NH") %>%
    group_by(p_taz) %>%
    summarize(nh_prods = sum(Weight_Factor))
  nh_attrs <- trips_df %>%
    filter(homebased == "NH") %>%
    group_by(a_taz) %>%
    summarize(nh_attrs = sum(Weight_Factor))
